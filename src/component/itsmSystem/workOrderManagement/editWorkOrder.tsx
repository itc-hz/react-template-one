...
const mapState = (state: IState) => {
  return {
    ...
    handlePerson: state.WorkOrderManagement.handlePerson,
    editShow: state.WorkOrderManagement.editShow,
    editType: state.WorkOrderManagement.editType,
    editId: state.WorkOrderManagement.editId,
    editForm: state.WorkOrderManagement.editForm,
...
const EditWorkOrder: React.ComponentType = (props: any) => {
  ...
  useEffect(() => {
    if (editType == 'edit') {
      _ITSMSystemServices.getWorkOrderMsg(editId, (res: any) => {
        dispatch({
  ...
  const onSubmit = () => {
    props.form.validateFields((err: any, vals: any) => {
      if (!err) {
        ...
        if (editType == 'add') {
          _ITSMSystemServices.addWorkOrder(data, (res: any) => {
            success("新建工单成功");
            dispatch({
        ...
        if (editType == 'edit') {
          _ITSMSystemServices.editWorkOrder(editId, data, (res: any) => {
            success("编辑工单成功");
            dispatch({
              type: "get workOrderManagement list",
              count: 0
  ...
  return (
    ...
    <Form {...formItemLayout}>
      ...
      <Form.Item label="工单来源">
        ...
        <Input style={{width: 250}} placeholder="请输入工单来源" />)}
      </Form.Item>
...
// 使表单组件具有props.form
const EditWorkOrderForm = Form.create({ name: 'EditWorkOrder' })(EditWorkOrder);
