...
const mapState = (state: IState) => {
  return {
    ...
    categorySetShow: state.ChangeManagement.categorySetShow,
  };
};
...
const CategorySet: React.ComponentType = (props: any) => {
  ...
  const addCategory = () => {
    props.form.validateFields(['addName'], (err: any, vals: any) => {
      if (!err) {
        _ITSMSystemServices.addChangeCategory({name: vals.addName}, (res: any) => {
          success("添加成功");
          getChangeCategory();
  ...
  const submitEditCategory = (id: any) => {
    props.form.validateFields([`editName${id}`], (err: any, vals: any) => {
      if (!err) {
        _ITSMSystemServices.editChangeCategory(id, {name: vals[`editName${id}`]}, (res: any) => {
  ...
  // 删除当前变更分类
  const deleteChangeCategory = (id: any) => {
    ...
    confirm({
      ...
      content: '是否删除当前所选分类？',
      onOk() {
        _ITSMSystemServices.deleteChangeCategory(id, (res: any) => {
          success("删除成功");
          getChangeCategory();
        }, (res: any) => {
          warning(res.message);
  ...
  const categoryItem = () => {
    return category.map((item: any, index: any) => (
      <Form.Item key={item.id}>
        ...
        <Button icon="delete" shape="circle" style={{marginLeft: 10}} onClick={() => deleteChangeCategory(item.id)}></Button>
      </Form.Item>
    ));
  };

  // 重置变更分类
  const resetChangeCategory = () => {
    ...
    confirm({
      ...
      content: '是否重置默认分类？',
      onOk() {
        _ITSMSystemServices.resetChangeCategory((res: any) => {
          success("重置成功");
          dispatch({
            type: "get changeManagement category",
            category: res.data
  ...
  return (
    ...
    <Button type="primary" style={{position: "absolute", top: -45, left: 100}} onClick={resetChangeCategory}>重置默认分类</Button>
  </Drawer>
  );
};
