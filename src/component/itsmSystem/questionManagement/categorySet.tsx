...
const mapState = (state: IState) => {
  return {
    ...
    categorySetShow: state.QuestionManagement.categorySetShow,
  };
};
...
const CategorySet: React.ComponentType = (props: any) => {
  ...
  const addCategory = () => {
    props.form.validateFields(['addName'], (err: any, vals: any) => {
      if (!err) {
        _ITSMSystemServices.addQuestionCategory({name: vals.addName}, (res: any) => {
          success("添加成功");
          getQuestionCategory();
  ...
  const submitEditCategory = (id: any) => {
    props.form.validateFields([`editName${id}`], (err: any, vals: any) => {
      if (!err) {
        _ITSMSystemServices.editQuestionCategory(id, {name: vals[`editName${id}`]}, (res: any) => {
  ...
  // 删除当前问题分类
  const deleteQuestionCategory = (id: any) => {
    ...
    confirm({
      ...
      content: '是否删除当前所选分类？',
      onOk() {
        _ITSMSystemServices.deleteQuestionCategory(id, (res: any) => {
          success("删除成功");
          getQuestionCategory();
        }, (res: any) => {
          warning(res.message);
  ...
  const categoryItem = () => {
    return category.map((item: any, index: any) => (
      <Form.Item key={item.id}>
        ...
        <Button icon="delete" shape="circle" style={{marginLeft: 10}} onClick={() => deleteQuestionCategory(item.id)}></Button>
      </Form.Item>
    ));
  };

  // 重置问题分类
  const resetQuestionCategory = () => {
    ...
    confirm({
      ...
      content: '是否重置默认分类？',
      onOk() {
        _ITSMSystemServices.resetQuestionCategory((res: any) => {
          success("重置成功");
          dispatch({
            type: "get questionManagement category",
            category: res.data
  ...
  return (
    ...
    <Button type="primary" style={{position: "absolute", top: -45, left: 100}} onClick={resetQuestionCategory}>重置默认分类</Button>
  </Drawer>
  );
};
